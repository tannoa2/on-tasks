...
function ipmiJobFactory(
    ...
    IpmiJob.prototype.genericCommand = function(data, command) {
        return ipmitool.generic(data.host, data.user, data.password,  command)
    };
    ...
    IpmiJob.prototype.collectIpmiSel = function(data) {
        ...
        .spread (function(selInfo, workObj){
            ...
            return self.genericCommand(data, "sel list first 1")
                .then(function(data){
                    var firstEntry = parseInt(data.split ("|")[0],16)
                    return firstEntry
                })
                .then(function(firstEntry){
                    //resting the counter when the the SEL log has been cleared
                    if(firstEntry < lastReportedSelEntryID){
                        lastReportedSelEntryID = firstEntry -1 ;
                    }
                    if(typeof(workObj.config.lastReportedSelEntryID) === 'undefined'){
                    //if(!(workObj.hasOwnProperty("config.lastReportedSelEntryID"))){
                        workObj.config.lastReportedSelEntryID = firstEntry + totalEntries;
                        workObj.save()
                        return;
                    }else{
                        stringifiedEntries = _.range(lastReportedSelEntryID ,( firstEntry+totalEntries + 1)).join(' ');
                        return self.getSelEntries(data, stringifiedEntries)
                            .tap(function() {
                                workObj.config.lastReportedSelEntryID = firstEntryfirstEntry + totalEntries ;
                                return workObj.save()
                            })
                    }
